...
# 2 "sha256_impl_tb.c" 2

# 1 "./sha256_impl_tb.h" 1
...
void sha256_update_tb(SHA256_CTX *ctx, unsigned char data1[], unsigned int len);
void sha256_final_tb(SHA256_CTX *ctx, unsigned char hash[]);
# 4 "sha256_impl_tb.c" 2
...
# 5 "sha256_impl_tb.c" 2
# 18 "sha256_impl_tb.c"
unsigned int k_tb[64] = {
...
void DBL_INT_ADD_tb(unsigned int a, unsigned int b, unsigned int c) {
   if (a > 0xffffffff - (c)) ++b; a += c;
   return;
}

unsigned int ROTRIGHT_tb(unsigned int a,unsigned int b){
 return (((a) >> (b)) | ((a) << (32-(b))));
}

unsigned int CH_tb(unsigned int x, unsigned int y, unsigned int z){
  return (((x) & (y)) ^ (~(x) & (z)));
}

unsigned int MAJ_tb(unsigned int x, unsigned int y,unsigned int z){
   return (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)));
}

unsigned int EP0_tb(unsigned int x){
   return (ROTRIGHT_tb(x,2) ^ ROTRIGHT_tb(x,13) ^ ROTRIGHT_tb(x,22));
}

unsigned int EP1_tb(unsigned int x){
   return (ROTRIGHT_tb(x,6) ^ ROTRIGHT_tb(x,11) ^ ROTRIGHT_tb(x,25));
}

unsigned int SIG0_tb(unsigned int x){
   return (ROTRIGHT_tb(x,7) ^ ROTRIGHT_tb(x,18) ^ ((x) >> 3));
}

unsigned int SIG1_tb(unsigned int x){
   return (ROTRIGHT_tb(x,17) ^ ROTRIGHT_tb(x,19) ^ ((x) >> 10));
}

void sha256_transform_tb(SHA256_CTX *ctx, unsigned char data[])
{
...
#pragma HLS pipeline II=2
 m[i] = SIG1_tb(m[i-2]) + m[i-7] + SIG0_tb(m[i-15]) + m[i-16];
   }
   a = ctx->state[0];
   b = ctx->state[1];
   c = ctx->state[2];
   ...
   for (i = 0; i < 64; ++i) {
#pragma HLS pipeline II=2
 t1 = h + EP1_tb(e) + CH_tb(e,f,g) + k_tb[i] + m[i];

   t2 = EP0_tb(a) + MAJ_tb(a,b,c);
      h = g;
      g = f;
      f = e;
      e = d + t1;
...
void sha256_update_tb(SHA256_CTX *ctx, unsigned char data[], unsigned int len)
{
   unsigned int i;
...
#pragma HLS loop_tripcount min=0 max=1
 ctx->data[ctx->datalen] = data[i];
      ctx->datalen++;
      if (ctx->datalen == 64) {
         sha256_transform_tb(ctx,ctx->data);
         DBL_INT_ADD_tb(ctx->bitlen[0],ctx->bitlen[1],512);
         ctx->datalen = 0;
      }
   }
}
...
#pragma HLS unroll factor=2
 ctx->data[i] = 0x00;
   ...
   DBL_INT_ADD_tb(ctx->bitlen[0],ctx->bitlen[1],ctx->datalen * 8);
